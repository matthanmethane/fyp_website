...
def run_tti_sim(
    ...
    max_symptomatic_tests_per_day = int(
        tests_per_day * max_pct_tests_for_symptomatics)
    ...
    if (checkpoints):
        ...
        for chkpt_param, chkpt_values in checkpoints.items():
            assert (isinstance(chkpt_values, (list, numpy.ndarray)) and len(chkpt_values) ==
        ...
        checkpointIdx = numpy.searchsorted(checkpoints['t'], model.t)
    ...
    while running:
        ...
        if (int(model.t) != int(timeOfLastIntervention)):
            ...
            if (backlog_skipped_intervals):
                cadenceDayNumbers = [int(i % cadence_cycle_length) for i in numpy.arange(
                    start=timeOfLastIntervention, stop=int(model.t), step=1.0)[1:]] + cadenceDayNumbers
            ...
            for cadenceDayNumber in cadenceDayNumbers:
                ...
                currentPctInfected = model.total_num_infected()[
                    model.tidx]/model.numNodes

                if (currentPctInfected >= intervention_start_pct_infected and not interventionOn):
                    interventionOn = True
                    interventionStartTime = model.t

                if (model.t >= interverntion_start_time and not interventionOn):
                    interventionOn = True
                    interventionStartTime = model.t
                ...
                if (interventionOn):
                    ...
                    if (any(isolation_compliance_symptomatic_individual)):
                        symptomaticNodes = numpy.argwhere(
                            (nodeStates == model.I_sym)).flatten()
                    ...
                    if (any(testing_compliance_symptomatic)):
                        ...
                        numSymptomaticTests = min(
                            len(symptomaticPool), max_symptomatic_tests_per_day)

                        if (len(symptomaticPool) > 0):
                            symptomaticSelection = symptomaticPool[numpy.random.choice(len(
                    ...
                    if (cadenceDayNumber in testingDays):
                        ...
                        if (any(testing_compliance_traced)):
                            ...
                            numTracingTests = min(len(tracingPool), min(
                                tests_per_day-len(symptomaticSelection), max_tracing_tests_per_day))
                        ...
                        if (any(testing_compliance_random)):
                            ...
                            numRandomTests = max(min(
                                tests_per_day-len(tracingSelection)-len(symptomaticSelection), len(testingPool)), 0)

                            testingPool_degrees = model.degree.flatten()[
                                testingPool]
                            testingPool_degreeWeights = numpy.power(testingPool_degrees, random_testing_degree_bias)/numpy.sum(
                                numpy.power(testingPool_degrees, random_testing_degree_bias))

                            if (len(testingPool) > 0):
                                randomSelection = testingPool[numpy.random.choice(
                        ...
                        # Also assume that latent infections are not picked up by tests:
                            ...
                            if (test_falseneg_rate == 'temporal'):
                                ...
                                if (testNodeState in list(temporal_falseneg_rates.keys())):
                                    falseneg_prob = temporal_falseneg_rates[testNodeState][int(min(
                                        testNodeTimeInState, max(list(temporal_falseneg_rates[testNodeState].keys()))))]
                                else:
                                    falseneg_prob = 1.00
                            else:
                    ...
                    isolationQueue_symptomatic.append(
                        newIsolationGroup_symptomatic)
                    ...
                    print("\t"+str(numTested_symptomatic) + "\ttested due to symptoms  [+ "+str(numPositive_symptomatic)+" positive (%.2f %%) +]" % (
                        numPositive_symptomatic/numTested_symptomatic*100 if numTested_symptomatic > 0 else 0))
                    print("\t"+str(numTested_tracing) + "\ttested as traces        [+ "+str(numPositive_tracing)+" positive (%.2f %%) +]" % (
                        numPositive_tracing/numTested_tracing*100 if numTested_tracing > 0 else 0))
                    print("\t"+str(numTested_random) + "\ttested randomly         [+ "+str(numPositive_random)+" positive (%.2f %%) +]" % (
                    ...
                    isolationGroup_symptomatic = isolationQueue_symptomatic.pop(
                        0)
